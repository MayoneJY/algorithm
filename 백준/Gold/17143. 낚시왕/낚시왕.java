...
public class Main {
    ...
    static void fishingShark(int x){
        for(int i = 1; i < R+1; i++){
            if(sharks[i][x] != null){
                ...
                // System.out.println(sharks[i][x].size);
                sharks[i][x] = null;
                break;
            }
        }
    ...
    //     List<Shark> kiledSharks = new ArrayList<>();
    //     for(int i = 0; i < sharks.size()-1; i++){
    //         Shark shark1 = sharks.get(i);
    //         for(int j = i; j < sharks.size(); j++){
    //             if(i == j) continue;
    //             Shark shark2 = sharks.get(j);
    //             if(shark1.x == shark2.x && shark1.y == shark2.y){
    //                 if(shark1.size > shark2.size)
    //                     kiledSharks.add(sharks.get(j));
    //                 else
    //                     kiledSharks.add(sharks.get(i));
    //             }
    //         }
    //     }
    //     for(int i = 0; i < kiledSharks.size(); i++){
    //         sharks.remove(kiledSharks.get(i));
    //     }
    // }
